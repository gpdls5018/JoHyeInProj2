...
public class FileUtils {
	...
	/**
	 * 다중 파일 업로드용 메소드로 파일을 하나라도 업로드 시에는 파일명이 저장된 StringBuffer반환
	 * 미 업로드시는 null반환
	 * 
		...
		try {			
			for(Part part:parts) {
				if(part.getContentType() !=null) {//input type="file"인 경우			
					...
					//파일명을 StringBuffer에저장
					fileNames.append(systemFileName+",");//구분자 ,로 지정
				}
			}
		}
		catch(Exception e) {//파일 미 업로드시 에러:null반환
			//e.printStackTrace();
			return null;
		}
	 ...
	 * @param webPath 파일이 저장된 서버의 웹상의 경로(webapp 아래)
	...
	public static void download(String filename, String webPath, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
				...
				response.setHeader("Content-Disposition", "attachment;filename="+filename);//반드시 이렇게 키,값을 설정하자
				...
				BufferedOutputStream bos =
					new BufferedOutputStream(response.getOutputStream());//연결된 OutputStream가져오기
