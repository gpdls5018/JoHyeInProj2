...
public class PagingUtil {
	...
	public static String pagingText(int totalRecordCount,int pageSize,int blockPage,int nowPage,String page){
		...
		if(intTemp != 1){
            //nowPage 상수 변경하기
			pagingStr+="<a class=\"page-link\" href='"+page+NOW_PAGE+"=1'>◀◀</a>&nbsp;<a class=\"page-link\" href='"+page+NOW_PAGE+"="+(intTemp -blockPage)+"'>◀</a>";
		}
		...
		while(blockCount <= blockPage && intTemp <= totalPage){  // 페이지 오버 를 체크
			...
			if(intTemp == nowPage){  
				pagingStr+="<span class=\"page-link text-danger\" >"+intTemp+"</span>&nbsp;";
		    	 ...
		    	 pagingStr+="<a class=\"page-link\" href='"+page+NOW_PAGE+"="+intTemp+"'>"+intTemp+"</a>&nbsp;";
		...
		if(intTemp <= totalPage){
			pagingStr+="<a class=\"page-link\" href='"+page+NOW_PAGE+"="+intTemp+"'>▶</a>&nbsp;<a class=\"page-link\" href='"+page+NOW_PAGE+"="+totalPage+"'>▶▶</a>"; 	   
		}
	 ...
	 * @param totalRecordCount 총 레코드 수
	 * @param pageSize 한 페이지에 보여줄 레코드 수
	 * @param blockPage 페이지싱 시 보여줄 페이지 번호의 수
	 * @param nowPage 사용자가 클릭한 페이지 번호
	 * @param page 페이징을 적용할 페이지명 (예: 페이지명?) 반드시 ?포함
	 * @return
	 */
	public static String pagingBootStrapStyle(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page){
		...
		if(intTemp != 1){
			pagingStr+="<li>\r\n" + 
					"<a  class=\"page-link\" href='"+page+NOW_PAGE+"=1'>\r\n" + 
					"<span >&laquo;</span>\r\n" + 
					"</a>\r\n" + 
					"</li>\r\n" + 
					"<li >\r\n" + 
					"<a class=\"page-link\" href='"+page+NOW_PAGE+"="+(intTemp -blockPage)+"' >\r\n" + 
					"<span>&lsaquo;</span>\r\n" + 
					"</a>\r\n" + 
					"</li>";
		}
		...
		while(blockCount <= blockPage && intTemp <= totalPage){  // 페이지 오버 를 체크
			...
			if(intTemp == nowPage){  
				pagingStr+="<li class=\"page-item\"><a class=\"page-link\" href='#'><span class=\"text-danger\">"+intTemp+"</span></a></li>";
		    	 ...
		    	 pagingStr+="<li class=\"page-item\"><a class=\"page-link\" href='"+page+NOW_PAGE+"="+intTemp+"'>"+intTemp+"</a></li>";
		...
		if(intTemp <= totalPage){
			pagingStr+="<li>\r\n" + 
					"<a class=\"page-link\" href='"+page+NOW_PAGE+"="+intTemp+"'>\r\n" + 
					"<span>&rsaquo;</span>\r\n" + 
					"</a>\r\n" + 
					"</li>\r\n" + 
					"<li class=\"page-item\">\r\n" + 
					"<a class=\"page-link\" href='"+page+NOW_PAGE+"="+totalPage+"' >\r\n" + 
					"<span >&raquo;</span>\r\n" + 
					"</a>\r\n" + 
					"</li>";		   
		}
	 ...
	 * @param servlet web.xml의 목록용 JSP페이지에 설정한 서블릿 초기화 파라미터를 가져오기 위한 HttpServlet객체
	 * 					JSP페이지에서는 this키워드를 넘기면 된다(jsp페이지가 서블릿이므로)
	 */
	...
	public static <T extends DaoService> void setMapForPaging(Map map, T dao, HttpServletRequest request, HttpServlet servlet) {
		...
		//페이지 사이즈와 블락페이지는 web.xml에 서블릿 초기화 파라미터로 설정해 놓은 값
		//페이지사이즈
		...
		int totalPage = (int)Math.ceil((double)totalRecordCount/pageSize);
		//현재 페이지 번호 : 초기화
		int nowPage =request.getParameter(NOW_PAGE)==null?1:Integer.parseInt(request.getParameter(NOW_PAGE));
		//시작 및 끝 ROWNUM구하기
